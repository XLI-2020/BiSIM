...
class Model(nn.Module):
    ...
    def build(self):
        ...
        self.decoder_cell = nn.LSTMCell(RNN_HID_SIZE+2, RNN_HID_SIZE)
        ...
        self.ATT = Attention(enc_hid_dim=RNN_HID_SIZE, dec_hid_dim=RNN_HID_SIZE)
    ...
    def forward(self, data, direct):
        ...
        for t in range(SEQ_LEN):
            ...
            # print('loss x - x_h', (torch.sum(torch.abs(x - x_h) * m) / (torch.sum(m) + 1e-5)).item())
            x_c = m * x + (1 - m) * x_h
            z_h = self.feat_reg(x_c)
            x_loss += torch.sum(torch.abs(x - z_h) * m) / (torch.sum(m) + 1e-5)
            # print('loss x - z_h', (torch.sum(torch.abs(x - z_h) * m) / (torch.sum(m) + 1e-5)).item())
            alpha = self.weight_combine(torch.cat([gamma_x,m], dim = 1))
            c_h = alpha * z_h + (1 - alpha) * x_h
            x_loss += torch.sum(torch.abs(x - c_h) * m) / (torch.sum(m) + 1e-5)
            c_c = m * x + (1 - m) * c_h
            # print('loss x - c_h', (torch.sum(torch.abs(x - c_h) * m) / (torch.sum(m) + 1e-5)).item())
            inputs = torch.cat([c_c, m], dim = 1)
            print('hhhhh shape', h.shape)
            enc_hid_output.append(h.unsqueeze(1)) #put decayed hiddened state into enc hid list
            h, c = self.rnn_cell(inputs, (h, c))
            imputations.append(c_c.unsqueeze(dim = 1))
        enc_hid_output = torch.cat(enc_hid_output, dim=1)
        print('enc_hid_output', enc_hid_output.shape)
        # enc_hid_output =  #(batch_size, seq_len, hidden_size)
        y_h = self.out(h)

...
wifi_df = pd.read_csv(os.path.join(derived_data_path, 'bfp_filterd_{site}_{floor}_{method}_{thre}.csv'.format(site=site, floor=floor_num, method=args.method, thre=args.thre)), header=0)
...
path_len_list = []
with open(os.path.join(brits_data_path, 'bt_biseq_{method}_{thre}.json'.format(method=args.method, thre=args.thre)), 'w+') as file:
    cnt = 0
    sum_y_labels = 0
    sum_eval_y_labels = 0
